...
public class TextUtil {
    ...
    public static String pluginPrefix() {
        if (!CommonUtil.getMajorVersion(16)) {
            return "§a[EnchantmentSlots]";
        }
        return "§x§9§8§F§B§9§8[EnchantmentSlots]";
    }
    ...
    public static String parse(String text) {
        return EnchantmentSlots.methodUtil.legacyParse(text);
    }
    ...
    public static void sendMessage(Player player, String rawText) {
        ...
        if (!rawText.contains("[")) {
            EnchantmentSlots.methodUtil.sendChat(player, rawText);
            return;
        }
        ...
        for (String msg : parseSimpleTag(rawText, "message")) {
            EnchantmentSlots.methodUtil.sendChat(player, msg);
            sentAny = true;
        }
        ...
        for (TagResult tag : parseArgTag(rawText, "title")) {
            ...
            EnchantmentSlots.methodUtil.sendTitle(
                    player,
                    data.title,
                    data.subTitle,
                    data.fadeIn,
        ...
        for (TagResult tag : parseArgTag(rawText, "bossbar")) {
            ...
            EnchantmentSlots.methodUtil.sendBossBar(
                    player,
                    data.title,
                    data.progress,
                    data.color,

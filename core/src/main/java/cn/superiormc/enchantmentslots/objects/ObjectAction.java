...
import cn.superiormc.enchantmentslots.objects.actions.ObjectSingleAction;
...
public class ObjectAction {
    ...
    public void runAllActions(Player player, int amount) {
        for (ObjectSingleAction singleAction : everyActions) {
            singleAction.doAction(player, amount);
        }
    }

    public void runRandomEveryActions(Player player, int amount, int x) {
        Collections.shuffle(everyActions);  // 随机打乱动作顺序
        for (int i = 0; i < Math.min(x, everyActions.size()); i++) {
            everyActions.get(i).doAction(player, amount);  // 执行 x 个随机动作
        }
    }

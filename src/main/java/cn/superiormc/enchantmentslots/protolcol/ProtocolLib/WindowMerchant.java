...
public class WindowMerchant extends GeneralPackets {
    ...
    protected void initPacketAdapter() {
        packetAdapter = new PacketAdapter(EnchantmentSlots.instance, ConfigReader.getPriority(), PacketType.Play.Server.OPEN_WINDOW_MERCHANT) {
            @Override
            public void onPacketSending(PacketEvent event) {
                ...
                packet.getMerchantRecipeLists().read(0).forEach(recipe -> {
                    ...
                    String itemID = CheckValidHook.checkValid(serverItemStack1);
                    int defaultSlot = ConfigReader.getDefaultLimits(event.getPlayer(), itemID);
                    ItemModify.addLore(serverItemStack1, defaultSlot, itemID);
                    ...
                    for (ItemStack serverItemStack2 : serverItemStack2List) {
                        String itemID2 = CheckValidHook.checkValid(serverItemStack2);
                        int defaultSlot2 = ConfigReader.getDefaultLimits(event.getPlayer(), itemID2);
                        ItemModify.addLore(serverItemStack2, defaultSlot2, itemID2);
